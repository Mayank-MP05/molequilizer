...
window.onload = function () {
  ...
  APP.display = new PeriodicTableDisplay(
    APP.periodictable,
    "periodictable",
    "infoboxbackground",
    "infobox"
  ...
  if (action == "remove") {
    for (let element of APP.periodictable._categoryCellData) {
      currentcell = document.querySelector(
        `[data-row='${element.tablerow18col}'][data-column='${element.tablecolumn18col}']`
      );
      currentcell.innerHTML = "";
      removeAttributes(currentcell, "class");
      currentcell.removeEventListener(
        "mouseover",
        listnerfunctions[element.category]
      );
    ...
    for (let element of APP.periodictable._categoryCellData) {
      ...
      listnerfunctions[element.category] = () =>
        applycategory(element.category);
...
function toggleBlockByCategory(action) {
  ...
  if (action == "remove") {
    for (let element of APP.periodictable._blockCellData) {
      ...
      currentcell.removeEventListener(
        "mouseover",
        listnerfunctions[element.block]
      );
